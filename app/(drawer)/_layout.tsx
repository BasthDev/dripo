...
import { isDashboardActive, isPosRouteActive, isRouteActive } from '../../utils/drawerNav';
...
function CustomDrawerContent(props: any) {
  ...
  const isMgmtActive = ['/products', '/categories', '/recipes', '/ingredients'].some((p) =>
    pathname.includes(p)
  );
  ...
  return (
    <View style={{ flex: 1 }}>
      <DrawerContentScrollView {...props} contentContainerStyle={{ flexGrow: 1, paddingBottom: Spacing.md }}>
        <View style={styles.drawerHeader}>
          ...
          <View>
            ...
            <Text style={styles.drawerSubtitle}>Make Every Sip Count</Text>
          </View>
        </View>
        ...
        <TouchableOpacity
          ...
          onPress={() => setManagementExpanded(!isManagementExpanded)}
          activeOpacity={0.7}
        >
          <View style={styles.dropdownHeaderLeft}>
            <Ionicons name="grid-outline" size={24} color={isMgmtActive ? Colors.primary : Colors.text} />
            <Text style={[styles.dropdownHeaderText, isMgmtActive && { color: Colors.primary }]}>
              Management
            </Text>
          </View>
          <Ionicons
            name={isManagementExpanded ? 'chevron-up' : 'chevron-down'}
            size={20}
            color={isMgmtActive ? Colors.primary : Colors.text}
          />
        </TouchableOpacity>
        ...
        {isManagementExpanded && (
          <View style={styles.dropdownContent}>
            <DrawerItem
              ...
              onPress={() => go('/(drawer)/products', isRouteActive(pathname, '/products'))}
            ...
            <DrawerItem
              label="Categories"
              icon={({ color, size }) => <Ionicons name="pricetags-outline" size={size} color={color} />}
              onPress={() => go('/(drawer)/categories', isRouteActive(pathname, '/categories'))}
            ...
            <DrawerItem
              label="HPP (Recipes)"
              icon={({ color, size }) => <Ionicons name="flask-outline" size={size} color={color} />}
              onPress={() => go('/(drawer)/recipes', isRouteActive(pathname, '/recipes'))}
            ...
            <DrawerItem
              ...
              onPress={() => go('/(drawer)/ingredients', isRouteActive(pathname, '/ingredients'))}
        ...
        <DrawerItem
          label="Sale"
          icon={({ color, size }) => <Ionicons name="bag-handle-outline" size={size} color={color} />}
        ...
        <DrawerItem
          label="Reports & Sales"
          icon={({ color, size }) => <Ionicons name="bar-chart-outline" size={size} color={color} />}
          onPress={() => go('/reports', isRouteActive(pathname, '/reports'))}
        ...
        <DrawerItem
          ...
          onPress={() => go('/settings', isRouteActive(pathname, '/settings'))}
      ...
      <View style={styles.drawerFooter}>
        <TouchableOpacity
          style={styles.rotateButtonFooter}
